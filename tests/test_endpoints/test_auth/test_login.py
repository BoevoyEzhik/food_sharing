...
async def test_login_user_success(mocker, client, sample_token):
    mock_service = mocker.patch('app.api.auth.endpoints.register.UserService.authenticate_user')
    mock_service.return_value = sample_token
    json_body = {'email': 'example@example.com',
