...
async def test_create_profile_info_success(
    mocker, client, sample_profile, profile_response
):
    mock_service = mocker.patch(
        "app.api.v1.create_profile.ProfileService.profile_create"
    )
...
async def test_create_profile_info_wrong_type(client):
    json_body = {
        "user_id": "9fa009ef-c9e8-4f33-a29c-d8879df07107",
        "title": 123,
...
async def test_create_profile_info_missed_required_field(client):
    json_body = {
        "description": "lastname",
        "birthday": "description",
        "active": True,
