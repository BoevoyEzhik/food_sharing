...
from app.models.schemas.register import RegisterInDB, RegisterIn
...
class UserService:
    ...
    async def register_user(self, user_info: RegisterIn) -> User:
        password_hash = await Password.create_hash_password(user_info.password)
        info_for_db = RegisterInDB(email=user_info.email,
    ...
    async def authenticate_user(self, login_info: Login) -> TokenDTO:
        ...
        if not await Password.is_valid_password(password=login_info.password,
    ...
    async def update_user_info(self, profile_info: Profile) -> User:
        user_dict = profile_info.dict()
        user_id = user_dict.pop('id')
        return await self.repo.update_user_info(user_id, user_dict)
